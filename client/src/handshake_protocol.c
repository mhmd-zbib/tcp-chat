...
int handshake_send_packet(int socket_fd, const handshake_packet_t *packet)
{
    ...
    LOG_HANDSHAKE("Sent %s packet (seq=%u, ack=%u)",
...
int handshake_receive_packet(int socket_fd, handshake_packet_t *packet)
{
    ...
    if (setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) < 0) {
        LOG_ERRNO("handshake_receive_packet: Failed to set socket timeout");
        return -1;
    }
    ...
    if (bytes_received != sizeof(handshake_packet_t)) {
        if (bytes_received < 0) {
            ...
            LOG_ERROR("handshake_receive_packet: Connection closed by peer");
        } else {
    ...
    LOG_HANDSHAKE("Received %s packet (seq=%u, ack=%u)",
                  packet->type == HANDSHAKE_TYPE_SYN       ? "SYN"
                  : packet->type == HANDSHAKE_TYPE_SYN_ACK ? "SYN-ACK"
                                                           : "ACK",
                  packet->seq_num, packet->ack_num);
...
{
    if (!nickname) {
        LOG_ERROR("handshake_perform_client_side: nickname is NULL");
        return -1;
    }

    LOG_HANDSHAKE("Starting 3-way handshake with server...");
    ...
    LOG_HANDSHAKE("Step 1: Sending SYN packet with nickname '%s'", nickname);
    if (handshake_send_packet(socket_fd, &syn_packet) < 0) {
        LOG_ERROR("handshake_perform_client_side: Failed to send SYN packet");
        return -1;
    }

    // Step 2: Receive SYN-ACK from server
    handshake_packet_t syn_ack_packet = {0};
    LOG_HANDSHAKE("Step 2: Waiting for SYN-ACK packet from server");
    if (handshake_receive_packet(socket_fd, &syn_ack_packet) < 0) {
        LOG_ERROR("handshake_perform_client_side: Failed to receive SYN-ACK packet");
        return -1;
    }

    if (syn_ack_packet.type != HANDSHAKE_TYPE_SYN_ACK) {
        LOG_ERROR("handshake_perform_client_side: Expected SYN-ACK packet, got type %d",
                  syn_ack_packet.type);
        return -1;
    }
    ...
    if (syn_ack_packet.ack_num != syn_packet.seq_num + 1) {
        LOG_ERROR("handshake_perform_client_side: Invalid ACK number (expected %u, got %u)",
    ...
    LOG_HANDSHAKE("Step 3: Sending ACK packet to complete handshake");
    if (handshake_send_packet(socket_fd, &ack_packet) < 0) {
        LOG_ERROR("handshake_perform_client_side: Failed to send ACK packet");
        return -1;
    }

    LOG_HANDSHAKE("3-way handshake completed successfully!");
    LOG_CONNECTION("Connected to server with nickname: %s", nickname);
